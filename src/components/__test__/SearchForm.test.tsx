...
describe('SearchForm Input Fields', () => {
  ...
  test('renders input fields with the correct labels and placeholders', () => {
    ...
    expect(
      screen.getByPlaceholderText(/Enter Minimum Age/i)
    ).toBeInTheDocument();

    expect(screen.getByText(/Maximum Age/i)).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText(/Enter Maximum Age/i)
    ).toBeInTheDocument();
  ...
  test('calls setFilter on submit', () => {
    ...
    fireEvent.change(screen.getByTestId('age-min-input'), {
      target: { value: '5' },
    });
    fireEvent.change(screen.getByTestId('age-max-input'), {
      target: { value: '10' },
    });
