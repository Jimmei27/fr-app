...
const SearchView = () => {
  ...
  // invoke request with search fields
    ...
    <div className="vh-100 d-flex flex-rows">
      <Col md={3} className="bg-light p-4">
        ...
        <FavoriteDogs
          ...
          onResetClick={() => setFavoriteBreeds([])}
        />
      </Col>
      <Col md={9} className="flex-fill">
        <div className="d-flex justify-content-end p-4">
          <SortDropdown handleSort={handleSort} />
          <Button onClick={() => logout()} size="sm" className="fr-button">
            Log Out
          </Button>
        </div>
        {dogData.length > 0 ? (
            ...
            <Row className="g-3 mx-3 row-cols-5">
              {dogData?.map((dog: Dog) => (
                <SearchDogCard
                  key={dog.id}
                  dog={dog}
                  onSelectDogBreed={onSelectDogBreed}
                />
              ))}
            </Row>
            <PagePagination
          ...
          <div className="d-flex justify-content-center mt-5">
            <h5>Please change the search fields and search again.</h5>
          </div>
        )}
      </Col>
